...
if flow_cfs and flow_cfs > 0:
    ...
    for idx, (lat, lon, mile) in enumerate(zip(marker_lats, marker_lons, marker_miles)):
        if mile in mile_markers:
            ...
            popup_content = f"<pre style='white-space: pre; font-family: monospace;'>Mile {mile}\nLat: {lat:.5f}\nLon: {lon:.5f}\nCFM: {cfm_at_mile:,}</pre>"
            folium.CircleMarker(
                location=[lat, lon],
                radius=6,
                color="green",
                ...
                fill_color="green",
                fill_opacity=0.8,
                tooltip=folium.Tooltip(popup_content, sticky=True, direction='right', permanent=False, max_width=800),
                popup=folium.Popup(popup_content, max_width=800)
            ).add_to(m)
    # Calculate flow at user's location (nearest mile marker)
    cfm_at_user = int(flow_cfm_initial * ((1 - loss_rate) ** nearest_marker))
    dam_popup_content = f"<pre style='white-space: pre; font-family: monospace;'>Old Hickory Dam\nLat: {nearest_lat:.5f}\nLon: {nearest_lon:.5f}</pre>"
    folium.CircleMarker(
        ...
        fill_color="red",
        fill_opacity=0.9,
        tooltip=folium.Tooltip(dam_popup_content, sticky=True, direction='right', permanent=False, max_width=800),
        popup=folium.Popup(dam_popup_content, max_width=800)
    ).add_to(m)
